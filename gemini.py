...
CODES_DIR = DATA_DIR / "generated_code"
...
TEXTS = {
    "en": {
        ...
        "menu_saved_codes": "7. Saved codes",
        "menu_exit": "8. Exit",
        "prompt_choice": "🔹 Choice (1‑8): ",
        ...
        "list_codes_item": "  {}. {} ({})",
        "code_actions": "Options: run (r), delete (d), back (b)",
        "code_choice_prompt": "Your choice (number or action): ",
        "code_run_which": "Enter number to run: ",
        "code_delete_which": "Enter number to delete: ",
        "code_deleted": "✅ Deleted '{}'",
        "code_cancelled": "Cancelled.",
        "code_run_again": "Running saved code...",
    },
    "vi": {
        ...
        "menu_saved_codes": "7. Code đã lưu",
        "menu_exit": "8. Thoát",
        "prompt_choice": "🔹 Lựa chọn (1‑8): ",
        ...
        "list_codes_item": "  {}. {} ({})",
        "code_actions": "Lựa chọn: chạy (r), xoá (d), quay lại (b)",
        "code_choice_prompt": "Lựa chọn của bạn (số hoặc hành động): ",
        "code_run_which": "Nhập số để chạy: ",
        "code_delete_which": "Nhập số để xoá: ",
        "code_deleted": "✅ Đã xoá '{}'",
        "code_cancelled": "Đã huỷ.",
        "code_run_again": "Đang chạy code đã lưu...",
    }
}
...
class GeminiChatbot:
    ...
    def _compile_code(self, compile_cmd: List[str], file_path: str, exe_path: str, tmp_dir: str, timeout: int) -> Optional[str]:
    ...
    def _prepare_run_command(self, run_cmd: List[str], file_path: str, exe_path: Optional[str], tmp_dir: str) -> List[str]:
        run_cmd_filled = []
        for arg in run_cmd:
            ...
            try:
                ...
                if compile_cmd:
                    if ext in ("java", "scala", "kotlin"):
                        ...
                        if os.name == "nt":
                            exe_path += ".exe"
                    comp_err = self._compile_code(compile_cmd, file_path, exe_path, tmp_dir, timeout)
                    if comp_err:
                        return comp_err

                run_cmd_filled = self._prepare_run_command(run_cmd, file_path, exe_path, tmp_dir)
    ...
    def list_saved_codes(self) -> List[Dict[str, Any]]:
        codes = []
        for file in sorted(CODES_DIR.glob("*"), key=lambda x: x.stat().st_mtime, reverse=True):
            if file.is_file():
    ...
    def run_saved_code(self, code_path: Path):
        ext = code_path.suffix[1:] if code_path.suffix else "txt"
        language = ext
        code = code_path.read_text(encoding="utf-8")
        print(f"{Fore.BLUE}{self.t('code_run_again')}{Style.RESET_ALL}")
        output = self.run_code(language, code)
        print(f"{Fore.GREEN}{self.t('code_output')}{Style.RESET_ALL}")
        print(output)

    def delete_saved_code(self, code_path: Path, name: str):
        confirm = input(self.t("delete_confirm", name)).strip().lower()
        if confirm == 'y':
            code_path.unlink()
            print(f"{Fore.GREEN}{self.t('code_deleted', name)}{Style.RESET_ALL}")
        else:
            print(f"{Fore.YELLOW}{self.t('code_cancelled')}{Style.RESET_ALL}")

    def manage_saved_codes(self):
        ...
        while True:
            print(f"\n{Fore.CYAN}{'='*60}")
            print(f"{Fore.MAGENTA}{self.t('saved_codes_title')}{Style.RESET_ALL}")
            print(f"{Fore.CYAN}{'='*60}")
            for i, c in enumerate(codes, 1):
                print(self.t('list_codes_item', i, c['name'], c['ext']))
            print(f"\n{Fore.YELLOW}{self.t('code_actions')}{Style.RESET_ALL}")
            choice = input(f"{Fore.GREEN}{self.t('code_choice_prompt')}{Style.RESET_ALL}").strip().lower()

            if choice == 'b':
                break
            elif choice == 'r':
                idx = input(self.t("code_run_which")).strip()
                if idx.isdigit() and 1 <= int(idx) <= len(codes):
                    self.run_saved_code(codes[int(idx)-1]['path'])
                else:
                    print(f"{Fore.RED}{self.t('invalid_choice')}{Style.RESET_ALL}")
            elif choice == 'd':
                idx = input(self.t("code_delete_which")).strip()
                if idx.isdigit() and 1 <= int(idx) <= len(codes):
                    c = codes[int(idx)-1]
    ...
    def main_menu(self):
        ...
        while True:
            ...
            print(f"{Fore.GREEN}{self.t('menu_saved_codes')}{Style.RESET_ALL}")
